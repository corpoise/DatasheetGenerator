...
`using Alias = Namespace.Type;` 형식의 using alias는 사용하지 않는다. Godot 타입과 System 타입이 충돌하는 경우, Godot API를 우선적으로 사용하여 충돌을 해소한다.

```csharp
// ✅
var dataPath = OS.GetEnvironment("MY_VAR");

// ❌
using Environment = System.Environment;
var dataPath = Environment.GetEnvironmentVariable("MY_VAR");
```
...
```csharp
// This is a mutable editor model, not a value object.
// It owns mutable Columns/Table state, so class is used instead of record.
public sealed class DatasheetDefinition
{
}
```
...
```csharp
// ✅
public TowerInfoComponent(TowerGameData gameData)
{
...
```csharp
// ✅
float speed = monster.GetSpeed(); // GetSpeed()가 이미 float 반환
return speed * PixelsPerUnit / this.radius;

// ❌
var speed = monster.GetSpeed(); // double 반환
return (float)speed * PixelsPerUnit / this.radius; // GetSpeed()를 float 반환으로 바꿔야 함
```
